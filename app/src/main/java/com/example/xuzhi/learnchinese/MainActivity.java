...
public void onLoadFinished(Loader<Cursor> cursorLoader, Cursor cursor) {
    ...
    public void  GenerateDefaultLearningList()
    {
        class LearningList
        {
            String name;
        ...
        for (int i = 0;i < primarySchool.length;i++) {
            value.put(LearnChineseContract.CustomLearning.COLUMN_NAME, primarySchool[i].name);
            value.put(LearnChineseContract.CustomLearning.COLUMN_DATE, Utility.getCurrentDate());
            value.put(LearnChineseContract.CustomLearning.COLUMN_CONTENT, primarySchool[i].content);
            value.put(LearnChineseContract.CustomLearning.COLUMN_STATUS, LearnChineseContract.NO);
            value.put(LearnChineseContract.CustomLearning.COLUMN_CHARACTER_SEQUENCE, primarySchool[i].characterIdSequence);

            getContentResolver().insert(LearnChineseContract.CustomLearning.CONTENT_URI, value);
        }
    }
}
